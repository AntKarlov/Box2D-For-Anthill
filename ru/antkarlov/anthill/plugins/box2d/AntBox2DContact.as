package ru.antkarlov.anthill.plugins.box2d
{
	...
	public class AntBox2DContact extends Object
	{
		...
		public function setData(aContact:b2Contact):void
		{
			...
			var m:b2Manifold = aContact.GetManifold();
			//var lp:b2Vec2 = m_points[0].m_localPoint;
			var gb:b2Vec2;
			var scale:Number = 0;
			if (collider.box2dBody != null)
			{
				gb = collider.box2dBody.GetWorldPoint(m.m_localPoint);
				scale = collider.manager.scale;
			}
			else if (collidee.box2dBody != null)
			{
				gb = collidee.box2dBody.GetWorldPoint(m.m_localPoint);
				scale = collidee.manager.scale;
			}
			
			if (gb != null)
			{
				positionX = gb.x * scale;
				positionY = gb.y * scale;
			}
